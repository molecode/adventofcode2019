...
def get_parameter_mode(parameter_modes, index):
    try:
        return parameter_modes[index]
    except IndexError:
        return 0

def get_input_one_two(parameter_modes, index):
...
while True:
    instruction = [x for x in map(int, str(intcode[index]))]
    ...
    if op_code == 1:
        input_one, input_two = get_input_one_two(parameter_modes, index)
